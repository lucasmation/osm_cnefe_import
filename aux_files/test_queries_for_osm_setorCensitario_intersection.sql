...
FROM OSM_Streets
INNER JOIN
        (SELECT *, ST_Buffer(geom,0.0005) as geom_buffed
...
EXPLAIN
CREATE TABLE OSM_Streets_by_SetorCensitarioD AS
SELECT osm_id, cd_geocodi as cod_setor
...
EXPLAIN
CREATE TABLE OSM_Streets_by_SetorCensitarioE AS
SELECT osm_id, cd_geocodi as cod_setor
FROM OSM_Streets_by_mun
INNER JOIN  setor_censitarioL2
ON 	OSM_Streets_by_mun.cod_mun=setor_censitarioL2.cod_mun AND
...
"Nested Loop  (cost=0.67..538425.89 rows=8560395 width=24)"
"  ->  Seq Scan on setor_censitariol2  (cost=0.00..119875.74 rows=316574 width=2156)"
"  ->  Nested Loop  (cost=0.67..1.31 rows=1 width=268)"
"        ->  Index Scan using osm_streets_by_mun_index3 on osm_streets_by_mun  (cost=0.41..1.03 rows=1 width=268)"
"              Index Cond: (((cod_mun)::text = setor_censitariol2.cod_mun) AND (way && setor_censitariol2.geom))"
"        ->  Function Scan on st_dwithin  (cost=0.26..0.27 rows=1 width=0)"
...
H   538425.89 rows=  8560395 width=24
...
H   119875.74 rows=   316574 width=2156
...
G  5757s= 96min = 1h36  , buffer 0.005   Query returned successfully: 22366081 rows affected, 5757770 ms execution time.
